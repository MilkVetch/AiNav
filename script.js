 ...
 * 包含：双语 I18N、Gist 同步、弹窗动画、样式隔离兼容
 */
...
const I18N = {
    zh: {
        ...
        setupBtn: "Setup Now", emptyBoard: "创建首个面板", confirmDelSite: "确认删除网址？", 
...
function render() {
        ...
        app.innerHTML = `
            <div class="welcome-container">
                <div class="welcome-card">
                    ...
                    <button class="glass-btn primary" onclick="handleOpenSetup()">
                        <i data-lucide="settings" class="icon-sm"></i> ${dict.setupBtn}
                    </button>
                </div>
                <div class="welcome-card">
                    <h4>📖 ${dict.tutorialTitle}</h4>
                    <div class="tutorial-steps-wrapper" style="flex-grow:1">
    ...
    if (!board) {
        app.innerHTML = `<button class="glass-btn primary" style="max-width:200px; margin: 2rem auto;" onclick="createNewBoard()">${dict.emptyBoard}</button>`;
        return;
    }
    ...
    board.categories.forEach((cat, cIdx) => {
        ...
        section.innerHTML = `
            <div class="menu-item-left" style="justify-content:space-between; margin-bottom:1.2rem; color:var(--text-dim); font-size:0.9rem">
                <span>${cat.name}</span>
                <button class="close-btn" style="font-size:0.8rem" onclick="deleteCat(${cIdx})">
                    <i data-lucide="trash-2" class="chevron-icon"></i>
                </button>
            </div>
            <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(150px, 1fr)); gap:1rem;" id="cat-${cIdx}"></div>
        `;
        ...
        cat.sites.forEach((site, sIdx) => {
            ...
            document.getElementById(`cat-${cIdx}`).innerHTML += `
                <a href="${site.url}" target="_blank" class="link-card">
                    <button class="close-btn" style="position:absolute; top:8px; right:8px; opacity:0; font-size:1.2rem;" onclick="event.preventDefault(); deleteSite(${cIdx}, ${sIdx})">
                        &times;
                    </button>
                    <img src="https://www.google.com/s2/favicons?sz=128&domain=${domain}" onerror="this.src='https://lucide.dev/favicon.ico'" style="width:32px; height:32px; border-radius:8px; margin-bottom:10px;">
                    <br><span style="font-size:0.85rem; font-weight:500; width:100%; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; display:block;">${site.name}</span>
                </a>
            `;
        });
    });
