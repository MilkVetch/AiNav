...
const I18N = {
    zh: {
        ...
        btnSave: "保存并同步", setupBtn: "开始配置", emptyBoard: "创建面板", confirmReset: "断开云端连接？", 
...
function updateClock() {
    ...
    const greetingEl = document.getElementById('greetingText'); const lang = db.lang || 'en';
    const currentKey = Object.keys(GREETINGS[lang]).sort().reverse().find(key => h >= parseInt(key.split(':')[0])) || "00:00";
    const target = GREETINGS[lang][currentKey];
    if (greetingEl.innerText !== target) {
        greetingEl.style.opacity = "0"; setTimeout(() => { greetingEl.innerText = target; greetingEl.style.opacity = "1"; }, 600);
    }
}
...
async function fetchData() {
    try {
        ...
        const gist = await res.json(); const content = JSON.parse(gist.files['ainav.json'].content);
...
function render() {
        ...
        app.innerHTML = `
            <div class="welcome-container">
                <div class="welcome-card">
                    ...
                    <button class="form-save-btn" onclick="handleOpenSetup()"><i data-lucide="settings" class="icon-sm"></i> ${dict.setupBtn}</button>
                </div>
                <div class="welcome-card">
                    ...
                    <button class="form-action-btn-sm" id="copyBtn" style="width:100%" onclick="copyInitialJSON()"><i data-lucide="copy" class="icon-sm"></i> ${dict.copyJsonBtn}</button>
                </div>
            </div>`;
    ...
    if (!board) { app.innerHTML = `<button class="form-save-btn" style="max-width:200px; margin: 2rem auto;" onclick="createNewBoard()">${dict.emptyBoard}</button>`; return; }
    ...
    board.categories.forEach((cat, cIdx) => {
        ...
        section.innerHTML = `<div class="menu-item-left" style="justify-content:space-between; margin-bottom:1rem; color:var(--text-dim)"><span>${cat.name}</span><button class="close-btn" style="font-size:0.8rem" onclick="deleteCat(${cIdx})"><i data-lucide="trash-2" class="chevron-icon"></i></button></div><div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(150px, 1fr)); gap:1rem;" id="cat-${cIdx}"></div>`;
        app.appendChild(section);
        cat.sites.forEach((site, sIdx) => {
            let domain = 'invalid'; try { domain = new URL(site.url).hostname; } catch(e) {}
            document.getElementById(`cat-${cIdx}`).innerHTML += `<a href="${site.url}" target="_blank" class="link-card"><button class="close-btn" style="position:absolute; top:5px; right:5px; opacity:0; font-size:1rem;" onclick="event.preventDefault(); deleteSite(${cIdx}, ${sIdx})">&times;</button><img src="https://www.google.com/s2/favicons?sz=128&domain=${domain}" onerror="this.src='https://lucide.dev/favicon.ico'" style="width:32px; margin-bottom:10px;"><br><span>${site.name}</span></a>`;
        });
    });
    lucide.createIcons();
}
...
function openCustomModal(id) { 
    ...
    if(id==='settingsModal') { document.getElementById('ghToken').value = CONFIG.token || ''; document.getElementById('gistId').value = CONFIG.gistId || ''; }
}
...
function saveSettings() {
    const token = document.getElementById('ghToken').value.trim(); const id = document.getElementById('gistId').value.trim();
    if (token) localStorage.setItem('gh_token', token); if (id) localStorage.setItem('gh_gist_id', id);
    location.reload();
}
...
async function pushToGist() {
    ...
    try {
        await fetch(`https://api.github.com/gists/${CONFIG.gistId}`, { method: 'PATCH', headers: { 'Authorization': `token ${CONFIG.token}` }, body: JSON.stringify({ files: { 'ainav.json': { content: JSON.stringify(db, null, 2) } } }) });
        updateStatus(true);
...
function copyInitialJSON() {
    ...
    navigator.clipboard.writeText(JSON.stringify(data, null, 2)).then(() => {
        const btn = document.getElementById('copyBtn'); const old = btn.innerHTML;
        btn.innerText = I18N[db.lang||'en'].copySuccess; setTimeout(() => btn.innerHTML = old, 2000);
    });
}
...
function addItem() { let url = document.getElementById('siteUrl').value.trim(); if (url && !/^https?:\/\//i.test(url)) url = 'https://' + url; const cIdx = document.getElementById('targetCat').value; const n = document.getElementById('siteName').value; if (cIdx !== "" && n && url) { db.boards[db.activeIndex].categories[cIdx].sites.push({ name: n, url: url }); closeAllModals(); render(); pushToGist(); } }
function addCategory() { const n = document.getElementById('catName').value.trim(); if(n) { db.boards[db.activeIndex].categories.push({name: n, sites: []}); render(); pushToGist(); closeAllModals(); } }
